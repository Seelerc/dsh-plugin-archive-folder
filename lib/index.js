...
export function apply(ctx) {
  ...
  // Read the complete raw event log for a session.
  //
  // NOT sessionQuery.readSession(): that path re-validates the log through
  // `Session.create(id, events, header, inheritedEventCount)`, and the
  // snapshot-mode constructor requires the seed to EQUAL the inherited prefix:
  //   if (mode === "snapshot" && header.isSeeded && inheritedEventCount !== log.length) throw
  // A fork/seed-born session (inheritedEventCount > 0) has a full log that is
  // LONGER than its inherited prefix, so readSession throws
  // "seeded session constructor seed must equal its inherited prefix" — which
  // is exactly the session left behind by this plugin's own restore button.
  // The live Session object exposes the same complete log without that check.
  async function fullLogOf(sessionId) {
    let live
    try { live = sessions.get(sessionId) } catch (e) { return null }
    if (!live) return null
    try {
      const events = live.snapshotEvents()
      if (!Array.isArray(events) || events.length === 0) return null
      return { header: live.header, inheritedEventCount: live.inheritedEventCount, events }
    } catch (e) { return null }
  }

  async function ensure(sessionId) {
    const log = (await fullLogOf(sessionId)) ?? await sessionQuery.readSession(sessionId)
    let title = 'chat'
    try {
  ...
  async function doStore(args) {
    ...
    try { info = await ensure(sessionId) } catch (e) {
      ...
      const hint = /seeded session constructor seed/.test(msg)
        ? '（该会话当前没有打开；请在要归档的那个会话里点「归档到文件夹」）'
        : ''
      return { ok: false, error: '读取会话失败: ' + msg + hint }
    }
    let base = info.cwd
    if (!base) {
      const existing = workspaceRegistry.list()
